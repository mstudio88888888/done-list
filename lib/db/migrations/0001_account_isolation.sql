-- Account boundary for every account-owned table.
-- The API must set app.current_account_id inside a transaction before querying.
ALTER TABLE tasks ENABLE ROW LEVEL SECURITY;
ALTER TABLE tasks FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS tasks_account_isolation ON tasks;
...
DROP POLICY IF EXISTS notebook_items_account_isolation ON notebook_items;
